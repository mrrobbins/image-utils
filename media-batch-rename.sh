...
# This script favors a date formatted filename. For each file, it attempts the following renaming strategies, based on the available metadata tags in the file, where last-one-wins:
...
## This approach is a substitute for a proper sub-second time field. The DateTimeOriginal tag does not record sub-second time and including it is optional and there is no standardized tag or value format used by manufactures.
...
# Note: the trailing "_0" in the filename, before the extension. This is a legacy digit meant to handle duplicate files. E.g. copies or edits of the same (captured) jpeg image.
# Removing this field and only adding it manually, on an as-needed basis, is possible.
